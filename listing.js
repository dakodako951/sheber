...
if(!listing||String(listing.id)!==String(requestedId)){document.querySelector('#listingMissing').hidden=false}else{
  ...
  const users=JSON.parse(localStorage.getItem('sheberUsers')||'[]'),sessionId=localStorage.getItem('sheberSession'),user=users.find(x=>x.id===sessionId),fav=document.querySelector('#pageFavorite');
  const key=user?`sheberFavorites_${user.id}`:null;let favorites=new Set(key?JSON.parse(localStorage.getItem(key)||'[]'):[]);
